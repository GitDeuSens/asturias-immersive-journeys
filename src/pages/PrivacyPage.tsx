...
const privacyContent = {
  es: {
    ...
    intro:
      "En cumplimiento del Reglamento (UE) 2016/679 del Parlamento Europeo y del Consejo (RGPD) y la Ley Orgánica 3/2018, de Protección de Datos Personales y garantía de los derechos digitales (LOPDGDD), le informamos sobre el tratamiento de sus datos personales.",
    controller: {
  ...
  en: {
    ...
    intro:
      "In compliance with Regulation (EU) 2016/679 of the European Parliament and Council (GDPR), we inform you about the processing of your personal data.",
    controller: {
  ...
  fr: {
    ...
    intro:
      "Conformément au Règlement (UE) 2016/679 du Parlement européen et du Conseil (RGPD), nous vous informons sur le traitement de vos données personnelles.",
    controller: {
...
export function PrivacyPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main id="main-content" className="flex-1 pt-14 md:pt-[122px]" role="main">
        <div className="container mx-auto px-4 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            {/* Intro */}
            <section className="mb-6 sm:mb-8 p-4 sm:p-6 bg-muted/50 rounded-xl">
            ...
            <section className="mb-8 p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.controller.title}</h2>
              <p className="text-muted-foreground whitespace-pre-line">{content.controller.text}</p>
            </section>

            {/* Purposes */}
            <section className="mb-8 p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.purposes.title}</h2>
            ...
            {/* Legal Basis */}
            <section className="mb-8 p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.legal.title}</h2>
              <p className="text-muted-foreground">{content.legal.text}</p>
            </section>

            {/* Data Processed */}
            <section className="mb-8 p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.data.title}</h2>
              <ul className="list-disc list-inside space-y-2 text-muted-foreground">
            ...
            {/* Retention */}
            <section className="mb-8 p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.retention.title}</h2>
              <p className="text-muted-foreground">{content.retention.text}</p>
            </section>

            {/* Rights */}
            <section className="mb-8 p-6 bg-primary/5 rounded-xl border border-primary/20">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.rights.title}</h2>
              <p className="text-muted-foreground mb-4">{content.rights.text}</p>
              <a
            ...
            <section className="p-6 bg-card border border-border rounded-xl">
              <h2 className="text-lg font-semibold text-foreground mb-3">{content.security.title}</h2>
              <p className="text-muted-foreground">{content.security.text}</p>
            </section>
          </motion.div>
        </div>
      </main>
