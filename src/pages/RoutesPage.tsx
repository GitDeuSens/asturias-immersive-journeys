...
import { MapPin, Search, ChevronUp, ChevronDown, Maximize2, Locate, Loader2, Smartphone, Camera, Info, Route as RouteIcon } from "lucide-react";
...
export const RoutesPage = React.memo(function RoutesPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      className="fixed right-0 bottom-0 w-full md:w-[480px] lg:w-[520px] md:max-w-xl bg-background z-[1] md:z-[35] shadow-2xl flex flex-col overflow-hidden top-14 md:top-[122px] md:rounded-tl-2xl md:border-l md:border-t md:border-border/40"
        >
          {/* Mobile handle */}
          <button
      ...
      {selectedPoint && (
        <PointDetailSheet
          ...
          allPoints={exploringRoute?.points || selectedRoute?.points}
          currentIndex={(exploringRoute?.points || selectedRoute?.points)?.findIndex(p => p.id === selectedPoint.id)}
          onNavigatePoint={handleSelectPoint}
        />
      )}
      </AnimatePresence>
    </div>
