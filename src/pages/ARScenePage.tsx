...
const texts = {
  ...
  instructions: {
    slam: {
      es: [
        'Asegúrate de estar en un espacio abierto con buena iluminación',
        'Permite el acceso a la cámara cuando se solicite',
        'Apunta la cámara al suelo y muévela lentamente',
        'Cuando aparezca un punto, toca para colocar el contenido'
      ],
      en: [
        'Make sure you are in an open space with good lighting',
        'Allow camera access when prompted',
        'Point the camera at the floor and move it slowly',
        'When a point appears, tap to place the content'
      ],
      fr: [
        'Assurez-vous d\'être dans un espace ouvert bien éclairé',
        'Autorisez l\'accès à la caméra lorsque demandé',
        'Pointez la caméra vers le sol et déplacez-la lentement',
        'Lorsqu\'un point apparaît, touchez pour placer le contenu'
      ]
    },
    'image-tracking': {
      es: [
        'Imprime el marcador en tamaño A4',
        'Colócalo en una superficie plana',
        'Apunta la cámara al marcador',
        'El contenido AR aparecerá sobre el marcador'
      ],
      en: [
        'Print the marker in A4 size',
        'Place it on a flat surface',
        'Point the camera at the marker',
        'The AR content will appear over the marker'
      ],
      fr: [
        'Imprimez le marqueur au format A4',
        'Placez-le sur une surface plane',
        'Pointez la caméra vers le marqueur',
        'Le contenu AR apparaîtra sur le marqueur'
      ]
    },
    geo: {
      es: [
        'Ve a la ubicación marcada en el mapa',
        'La experiencia AR se activará automáticamente al acercarte',
        'Permite el acceso a tu ubicación cuando se solicite'
      ],
      en: [
        'Go to the location marked on the map',
        'The AR experience will activate automatically when you approach',
        'Allow location access when prompted'
      ],
      fr: [
        'Rendez-vous à l\'emplacement indiqué sur la carte',
        'L\'expérience AR s\'activera automatiquement à l\'approche',
        'Autorisez l\'accès à votre position lorsque demandé'
      ]
    }
  },
  ...
  useEffect(() => {
    async function loadScene() {
      ...
      try {
        ...
        if (data) {
          trackEvent('ar_scene_viewed', { ar_id: data.id, ar_slug: slug });
        }
      } catch (err) {
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="pt-20 flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <h1 className="text-xl font-semibold text-foreground mb-2">
            {texts.notFound[locale as Language]}
          </h1>
          <Link to="/">
            <Button variant="outline">
              <ArrowLeft className="w-4 h-4 mr-2" />
              {texts.back[locale as Language]}
            </Button>
          </Link>
        </div>
      </div>
          ...
          >
            {scene.preview_video ? (
              ...
              <img
                ...
                alt={scene.title[locale as Language] || scene.title.es}
                className="w-full aspect-video object-cover"
              />
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                <Badge variant="secondary" className="bg-card/90">
                  ...
                  {scene.duration_minutes} {texts.duration[locale as Language]}
                </Badge>
                <Badge variant="secondary" className="bg-card/90">
                  {texts.arType[scene.needle_type][locale as Language]}
                </Badge>
                <Badge variant="secondary" className="bg-card/90">
                  {texts.difficulty[scene.difficulty][locale as Language]}
                </Badge>
              </div>
              ...
              <p className="text-white/80">
                {scene.description[locale as Language] || scene.description.es}
              </p>
            </div>
          </motion.div>
            ...
            <div className="md:col-span-2 space-y-8">
              ...
              <section className="bg-card border border-border rounded-xl p-6">
                <h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  ...
                  {texts.howToUse[locale as Language]}
                </h2>
                ...
                {/* Tracking marker for image-tracking */}
                {scene.needle_type === 'image-tracking' && scene.tracking_image_url && (
                  <div className="mt-6 pt-6 border-t border-border">
                    <p className="text-sm text-muted-foreground mb-3">
                      {locale === 'es' ? 'Marcador para imprimir:' :
                       locale === 'en' ? 'Marker to print:' :
                       'Marqueur à imprimer:'}
                    </p>
                    <img
                      src={scene.tracking_image_url}
                      alt="AR Marker"
                      className="w-32 h-32 border border-border rounded-lg mb-3"
                    />
                    <a
                      href={scene.tracking_image_url}
                      download={`marcador-${scene.slug}.png`}
                      className="inline-flex items-center text-sm text-primary hover:underline"
                    >
                      <Download className="w-4 h-4 mr-1" />
                      {locale === 'es' ? 'Descargar marcador (A4)' :
                       locale === 'en' ? 'Download marker (A4)' :
                       'Télécharger le marqueur (A4)'}
                    </a>
                  </div>
                )}
                ...
                {scene.needle_type === 'geo' && scene.location && (
                  <div className="mt-6 pt-6 border-t border-border">
                    ...
                    >
                      ...
                      {locale === 'es' ? 'Cómo llegar' :
                       locale === 'en' ? 'Get directions' :
                       'Itinéraire'}
                    </Button>
                  </div>
                )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card border border-border rounded-xl p-6 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {locale === 'es' ? 'Escanea para abrir en tu móvil' :
                   locale === 'en' ? 'Scan to open on your phone' :
                   'Scannez pour ouvrir sur votre téléphone'}
                </p>
                <Button variant="outline" size="sm" onClick={handleDownloadQR} className="w-full">
                  <Download className="w-4 h-4 mr-2" />
                  {texts.downloadQR[locale as Language]}
                </Button>
              </div>
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-medium text-foreground mb-4 flex items-center gap-2">
                  ...
                  {texts.shareExperience[locale as Language]}
                </h3>
                <ShareButtons
                  url={currentUrl}
                  title={scene.title[locale as Language] || scene.title.es}
              ...
              <div className="bg-muted/50 border border-border rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {locale === 'es' 
                      ? 'Esta experiencia funciona mejor en dispositivos móviles con ARCore (Android) o ARKit (iOS).'
                      : locale === 'en'
                      ? 'This experience works best on mobile devices with ARCore (Android) or ARKit (iOS).'
