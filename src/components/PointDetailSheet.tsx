...
import {
  X, MapPin, Camera, Play, FileText, Headphones, Smartphone,
  ExternalLink, ChevronRight, Maximize2, Sparkles, ScanLine,
  Image as ImageIcon, Phone, Mail, Globe, Clock, Euro, Info,
  Navigation, Footprints, Car
} from 'lucide-react';
...
// Safe multilingual text extractor
...
export function PointDetailSheet({ point, onClose }: PointDetailSheetProps) {
  ...
  useEffect(() => {
    return () => {
      if (point) {
        ...
        if (durationSec > 2) {
          trackPOITimeSpent(point.id, getText(point.title, language), durationSec);
        }
      }
    };
  }, [point, language]);
  ...
  const handleNavigateToStart = () => {
    if (point.location !== null) {
      openNavigation(point.location.lat, point.location.lng, title);
    }
  };
        ...
        >
          ...
          <ScrollArea className="flex-1">
            ...
            <div className="p-6 space-y-6">
              <p className="text-muted-foreground leading-relaxed text-base">{shortDescription}</p>
              ...
              {hasAR && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-4">
                    ...
                    <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-white/20 rounded-lg"><Smartphone className="w-6 h-6" /></div>
                      ...
                      <div className="flex flex-col items-center text-center space-y-5">
                        <div className="relative">
                          ...
                          <div className="relative p-5 bg-white rounded-2xl shadow-xl border-4 border-warm/30">
                            <QRCodeSVG value={content.arExperience!.qrValue} size={200} level="H" includeMargin={false} bgColor="white" fgColor="#1a1a1a" />
                          </div>
                          <motion.div
                        ...
                        <div className="space-y-2">
                          <p className="font-semibold text-foreground text-lg flex items-center justify-center gap-2">
                            <Smartphone className="w-5 h-5 text-warm" />{t(texts.scanQR)}
                          </p>
                        ...
                        <div className="w-full pt-4 border-t border-border/50">
                          ...
                          <Button variant="outline" onClick={() => setShowARFullscreen(true)} className="w-full border-warm/30 text-warm hover:bg-warm/10 hover:text-warm">
                            <Maximize2 className="w-4 h-4 mr-2" />{t(texts.tryARDesktop)}
                          </Button>
                        </div>
                      </div>
                    </div>
                  ...
                  {content.arExperience?.instructions && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="p-4 rounded-xl bg-warm/10 border border-warm/20">
                      <p className="text-xs font-semibold text-warm uppercase tracking-wide mb-2 flex items-center gap-2">
                        <Sparkles className="w-3 h-3" />{t(texts.arInstructions)}
                      </p>
                  ...
                  <div className="space-y-2">
                    {hasVideo && (
                      <Button variant="outline" className="w-full justify-between" onClick={() => window.open(content.video!.url, '_blank')}>
                        <span className="flex items-center gap-2"><Play className="w-4 h-4" />{t(texts.playVideo)}</span>
                    ...
                    {hasAudio && content.audioGuide?.[language as keyof typeof content.audioGuide] && (
                      <Button variant="outline" className="w-full justify-between" onClick={() => window.open(content.audioGuide![language as keyof typeof content.audioGuide]!.url, '_blank')}>
                        <span className="flex items-center gap-2"><Headphones className="w-4 h-4" />{t(texts.listenAudio)}</span>
                        <ChevronRight className="w-4 h-4" />
                      </Button>
                    )}
                    {hasPDF && (
                      <Button variant="outline" className="w-full justify-between" onClick={() => window.open(content.pdf!.url, '_blank')}>
                        <span className="flex items-center gap-2"><FileText className="w-4 h-4" />{t(texts.downloadPDF)}</span>
                        <ChevronRight className="w-4 h-4" />
                      </Button>
                    )}
                  </div>
                    ...
                    {(content.practicalInfo?.phone || content.practicalInfo?.email || content.practicalInfo?.website) && (
                      <div className="space-y-2">
                        {content.practicalInfo?.phone && (
                          <a href={`tel:${content.practicalInfo.phone}`} className="flex items-center gap-3 text-sm text-foreground hover:text-primary transition-colors">
                            <Phone className="w-4 h-4 text-muted-foreground" />{content.practicalInfo.phone}
                          </a>
                        )}
                        {content.practicalInfo?.email && (
                          <a href={`mailto:${content.practicalInfo.email}`} className="flex items-center gap-3 text-sm text-foreground hover:text-primary transition-colors">
                            <Mail className="w-4 h-4 text-muted-foreground" />{content.practicalInfo.email}
                          </a>
                        )}
                        {content.practicalInfo?.website && (
                          <a href={content.practicalInfo.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-foreground hover:text-primary transition-colors">
                            <Globe className="w-4 h-4 text-muted-foreground" />{content.practicalInfo.website.replace(/^https?:\/\//, '').replace(/\/$/, '')}
                          </a>
                        )}
                      </div>
                    )}
                    {content.practicalInfo?.schedule && (
                      <div className="pt-2 border-t border-border/50">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">{t(texts.schedule)}</p>
                            <p className="text-sm text-foreground whitespace-pre-line">{t(content.practicalInfo.schedule)}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    {content.practicalInfo?.prices && (
                      <div className="pt-2 border-t border-border/50">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">{t(texts.prices)}</p>
                            <p className="text-sm text-foreground whitespace-pre-line">{t(content.practicalInfo.prices)}</p>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        {selectedGalleryImage && (
          ...
          >
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative max-w-4xl max-h-[80vh] w-full">
              ...
              <button onClick={() => setSelectedGalleryImage(null)} className="absolute top-4 right-4 p-2 rounded-full bg-black/50 text-white hover:bg-black/70 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </motion.div>
          </motion.div>
        ...
        {showARViewer && arScene && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black">
            <NeedleARViewer
              ...
              onError={() => setShowARViewer(false)}
            />
            <button onClick={() => setShowARViewer(false)} className="absolute top-4 right-4 p-2 rounded-full bg-black/50 text-white hover:bg-black/70 transition-colors z-[80]">
              <X className="w-5 h-5" />
            </button>
          </motion.div>
        )}
...
function NavigationSection({ point }: { point: RoutePoint }) {
  ...
  const destination: NavigationDestination = {
    id: point.id,
    name: getText(point.title, language),
      ...
      <div className="flex items-center gap-4 mb-4 p-3 rounded-lg bg-card/50">
        ...
        <div className="flex-1 grid grid-cols-2 gap-2 text-sm">
          <div className="flex items-center gap-1.5 text-muted-foreground">
            <Footprints className="w-4 h-4" /><span>{formatTime(distanceResult.estimatedWalkingTime)}</span>
          </div>
          <div className="flex items-center gap-1.5 text-muted-foreground">
            <Car className="w-4 h-4" /><span>{formatTime(distanceResult.estimatedDrivingTime)}</span>
          </div>
        </div>
      </div>
