...
const NeedleARViewer = lazy(() => import('@/components/NeedleARViewer'));
    ...
    <>
      <AnimatePresence>
        <motion.div key="overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="fixed inset-0 z-[46] bg-black/30 pointer-events-auto" onClick={onClose} />
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-6 pt-3 pb-1">
                <Breadcrumb>
                  <BreadcrumbList>
                    ...
                    <BreadcrumbItem>
                      {onBackToRoute ? (
                        <BreadcrumbLink href="#" onClick={(e) => { e.preventDefault(); onBackToRoute(); }} className="text-xs truncate max-w-[80px] sm:max-w-[140px] block">
                          {routeTitle}
                        </BreadcrumbLink>
                      ) : (
                        <span className="text-xs text-muted-foreground truncate max-w-[80px] sm:max-w-[140px] block">{routeTitle}</span>
                      )}
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                      <BreadcrumbPage className="text-xs truncate max-w-[80px] sm:max-w-[140px] block">{title}</BreadcrumbPage>
                    </BreadcrumbItem>
                  </BreadcrumbList>
                </Breadcrumb>
              </div>
            ...
            {allPoints && allPoints.length > 1 && currentIndex !== undefined && onNavigatePoint && (
              <div className="flex justify-center pt-2 pb-2">
                <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2 w-full max-w-[240px]">
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    ...
                    <span className="text-xs">{language === 'es' ? 'Prev' : 'Prev'}</span>
                  </Button>
                  <span className="justify-self-center text-xs text-muted-foreground font-medium tabular-nums">
                  ...
                  <Button
                    ...
                    onClick={() => onNavigatePoint(allPoints[currentIndex + 1])}
                  >
                    <span className="text-xs">{language === 'es' ? 'Next' : 'Next'}</span>
