...
const fallbackTexts = {
  ...
  toursCta: { es: 'Explorar →', en: 'Explore →', fr: 'Explorer →' },
  routesCta: { es: 'Explorar →', en: 'Explore →', fr: 'Explorer →' },
  homeDescription: {
...
export function ExperienceSelector({ onSelect, onBack }: ExperienceSelectorProps) {
  ...
  const title = pick(config?.skip_link, fallbackTexts.title, lang) || t(fallbackTexts.title);
  const description = pick(config?.card1_description, fallbackTexts.homeDescription, lang);

  // Detail cards from CMS
  const toursHeading = pick(config?.detail1_heading, fallbackTexts.toursTitle, lang);
  const toursDescription = pick(config?.detail1_description, fallbackTexts.toursSubtitle, lang);
  const toursCta = pick(config?.detail1_cta, fallbackTexts.toursCta, lang);
  const toursBg = config?.detail1_bg_image ? getDirectusAssetUrl(config.detail1_bg_image) : null;

  const routesHeading = pick(config?.detail2_heading, fallbackTexts.routesTitle, lang);
  const routesDescription = pick(config?.detail2_description, fallbackTexts.routesSubtitle, lang);
  const routesCta = pick(config?.detail2_cta, fallbackTexts.routesCta, lang);
  const routesBg = config?.detail2_bg_image ? getDirectusAssetUrl(config.detail2_bg_image) : null;

  return (
    <>
      <div className="experiences-container relative z-10 flex flex-col items-center justify-center px-4 py-3 min-h-dvh">
        {/* Title */}
        <motion.h1
        ...
        <span style={{ fontWeight: 'bold' }} className="description text-white mb-4 sm:mb-8 text-xs sm:text-base text-center px-2 leading-relaxed max-w-3xl">{description}</span>
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-6 w-full max-w-4xl">
          ...
          >
            {routesBg && (
              <div
                className="absolute inset-0 bg-cover bg-center opacity-30 group-hover:opacity-40 transition-opacity"
                style={{ backgroundImage: `url(${routesBg})` }}
              />
            )}
            <div className="relative z-10">
        ...
        >
          ...
          <span className="font-medium text-xl">{t(fallbackTexts.back)}</span>
        </motion.button>
        )}
      </div>
      <Footer />
