  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {/* Theme */}
        <p className="text-xs text-primary font-medium mb-2">{route.theme[language]}</p>

        {/* Description */}
        <p className="text-sm text-muted-foreground mb-3 line-clamp-2">{route.shortDescription[language]}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {route.elevationGainMeters && route.elevationGainMeters > 0 && (
            <span className="flex items-center gap-1 text-xs text-muted-foreground font-medium">
              <Mountain className="w-3 h-3" />↑{route.elevationGainMeters} m
            </span>
          )}
