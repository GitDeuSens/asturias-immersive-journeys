...
function transformTour360(tour: DirectusTour360): KuulaTour {
  // slug = custom URL slug from Directus (e.g. "tineo-moa"), stripped of leading "/"
  const rawSlug = ((tour as any).slug || '') as string;
    ...
    try {
      const [routes, analyticsCounts] = await Promise.all([
        this.getClient().request(readItems('routes', {
          filter: { slug: { _eq: slug }, status: { _in: API_CONFIG.getStatusFilter() } },
