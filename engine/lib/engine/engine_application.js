...
export function internalOnUserInputRegistered() {
    if (userInteractionRegistered)
        return;
    if (isDevEnvironment())
        console.debug("[Needle Engine] User input registered: Media playback is now allowed.");
...
if (typeof window !== "undefined" && "userActivation" in navigator && navigator.userActivation?.isActive) {
    if (isDevEnvironment())
        console.debug("[Needle Engine] User input already active: Media playback is now allowed.");
    userInteractionCallbacks.length = 0;
    userInteractionRegistered = true;
}
/**
