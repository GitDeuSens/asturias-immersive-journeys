    ...
    */
    ...
    static async start(mode, init, context) {
        ...
        const newSession = await (this._currentSessionRequest)?.catch(e => {
            ...
            if (e?.code === 9)
                showBalloonWarning("Couldn't start XR session. Make sure you allow the required permissions.");
    ...
    static invokeSessionRequestStart(mode, init) {
        for (const listener of this._sessionRequestStartListeners) {
            listener({ mode, init });
        }
    }
    static invokeSessionRequestEnd(mode, init, session) {
