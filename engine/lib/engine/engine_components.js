 ...
 * ```
 */
export function getComponentInChildren(obj, componentType, includeInactive = false) {
    if (includeInactive === false && obj[activeInHierarchyFieldName] === false)
        return null;
    const res = getComponent(obj, componentType);
    if (includeInactive === false && (res?.enabled === false || res?.activeAndEnabled === false))
        return null;
    if (res)
        return res;
 ...
 * ```
 */
export function getComponentInParent(obj, componentType, includeInactive = false) {
    if (!obj)
        return null;
    if (Array.isArray(obj)) {
        for (let i = 0; i < obj.length; i++) {
            const o = tryGetObject(obj[i]);
            const res = getComponentInParent(o, componentType, includeInactive);
            if (res)
                return res;
        }
        return null;
    ...
    if (res)
        return res;
    if (obj.parent)
        return getComponentInParent(obj.parent, componentType, includeInactive);
    return null;
}
/**
 * Searches for a given component type in the parent hierarchy of the given object.
