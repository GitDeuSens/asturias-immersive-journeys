...
export class USDZExporter extends Behaviour {
    ...
    static beforeLODExport = new EventList();
    /**
    ...
    async internalExport(objectToExport) {
        ...
        for (const rend of renderers) {
            for (const mesh of rend.sharedMeshes) {
                if (mesh) {
                    let lodLevel = 0;
                    const args = { exporter: this, type: "mesh", object: rend.gameObject, mesh: mesh, };
                    USDZExporter.beforeLODExport.invoke(args);
                    if (args.overrideLevel !== undefined) {
                        if (args.overrideLevel === -1) {
                            if (debug)
                                console.warn("Skipping LOD export for mesh due to overrideLevel -1", rend.gameObject, mesh);
                            continue; // skip LOD assignment
                        }
                        else if (args.overrideLevel >= 0) {
                            lodLevel = args.overrideLevel;
                            if (debug)
                                console.log("Overriding LOD level for mesh export to level " + lodLevel + " " + mesh.name);
                        }
                    }
            ...
            for (const mat of rend.sharedMaterials) {
                if (mat) {
                    let lodLevel = 0;
                    const args = { exporter: this, type: "texture", object: rend.gameObject, material: mat };
                    USDZExporter.beforeLODExport.invoke(args);
                    if (args.overrideLevel !== undefined) {
                        if (args.overrideLevel === -1) {
                            if (debug)
                                console.warn("Skipping LOD assignment due to overrideLevel -1", rend.gameObject, mat);
                            continue; // skip LOD assignment
                        }
                        else if (args.overrideLevel >= 0) {
                            lodLevel = args.overrideLevel;
                            if (debug)
                                console.log("Overriding LOD level for texture export to level " + lodLevel + " " + mat.name);
                        }
                    }
