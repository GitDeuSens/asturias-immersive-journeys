...
export class SceneSwitcher extends Behaviour {
    ...
    */
    ...
    select(index) {
        if (debug)
            console.log("[SceneSwitcher] select", index);
    ...
    async __unloadCurrentScene() {
        ...
        if (current) {
            if (debug)
                console.log("[SceneSwitcher] UNLOAD", current.url, "HasURL?: " + current.hasUrl);
            ...
            if (current.hasUrl) {
                ...
                if (this._currentSceneAsset)
                    destroy(this._currentSceneAsset, true, false);
            }
            else {
    ...
    async __internalSwitchScene(scene) {
        ...
        try {
            if (debug)
                console.debug(`${Date.now()} [SceneSwitcher] Loading scene start: ${scene.url} (index: ${index})`);
            ...
            if (finishedEvt.defaultPrevented) {
                if (debug)
                    console.warn("[SceneSwitcher] Adding loaded scene prevented:", scene, finishedEvt);
                return false;
            }
            if (!scene.asset) {
                if (debug)
                    console.warn("[SceneSwitcher] Failed loading scene:", scene);
                return false;
            }
            if (this._currentIndex === index) {
                if (debug)
                    console.log("[SceneSwitcher] ADD", scene.url);
                ...
                if (debug)
                    console.debug(`${Date.now()} [SceneSwitcher] Loading scene finished: ${scene.url} (index: ${index})`);
    ...
    trySelectSceneFromValue(value) {
        ...
        if (isLocalNetwork())
            console.warn("[SceneSwitcher] Can not find scene: \"" + value + "\"", this);
        return couldNotLoadScenePromise;
    }
...
class PreLoadScheduler {
    ...
    begin(delay) {
        if (this._isRunning)
            return;
        if (debug)
            console.log("[SceneSwitcher] Preload scheduled", { delay });
        ...
        const interval = setInterval(() => {
            ...
            if (!this._loadTasks.some(t => t.index === roomIndex)) {
                ...
                if (debug)
                    console.log("[SceneSwitcher] Schedule preload scene", { roomIndex, searchForward, lastRoom, currentIndex: this._switcher.currentIndex, tasks: this._loadTasks.length }, scene?.url);
